...
export default {
	name: 'aktivity-guide-25-senat-leden',
	props: [],
	data: function () {
		return {
			town: null,
	...
	computed: {
		...
		activity: function () {
			...
			if (this.parties) {
				...
				this.partyList.forEach(party => {
					...
					var obvod = this.$store.getters.pdv('elections/activity/senat/171:' + party);
		...
		senatData: function () {
			var list = [];
			var detail = [];
			var data = this.inSenat ? this.$store.getters.pdv('elections/fetch/171:' + this.obvodID) : null;
			var previous = this.$store.getters.pdv('elections/fetch/143:' + this.obvodID);
			var answers = [];
	...
	mounted: function () {
	  window.scrollTo(0, 1);
	  ga('Průvodce senátními volbami 2025');
