...
export default {
  ...
  components: {
	...
	computed: {
		...
		obvody: function () {
			return this.current.$obce.find(x => x.OBVODY === 2) ? this.current.$obce.filter(x => x.OBVODY === 2 && x.COBVODU) : this.current.$obce
		}
	},
  methods: {
		url,
		...
		sortByVotes: function (list, parties, obvod) {
			...
			list.filter(x => x.OBVOD === obvod).filter(x => x.MANDATU).forEach(x => arr.push(x));
			list.filter(x => x.OBVOD === obvod).filter(x => !x.MANDATU).forEach(x => arr2.push(x));
		...
		parties: function (list, parties) {
			...
			list.forEach(item => {
				...
				var o = {
					...
					short: d.ZKRATKA,
					link: '/volby/komunalni-volby/' + this.id + '/strana/' + this.current.$strany.filter(x => x.OSTRANA === d.OSTRANA && x.KODZASTUP === d.KODZASTUP).map(x => x.id).join(','),
