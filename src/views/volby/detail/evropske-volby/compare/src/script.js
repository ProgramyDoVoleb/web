...
export default {
	...
	computed: {
			...
			arr.forEach(party => {
				...
				this.previous.cis.strany.filter(x => x.VSTRANA === party.VSTRANA).forEach(res => {
					...
					if (runs) {
						...
						party.previous = {
							party: res,
							result: this.previous.list[0].$vysledky.find(x => x.ESTRANA === runs.ESTRANA),
							list: this.previous.list[0].$zvoleni.filter(x => x.ESTRANA === runs.ESTRANA),
							runs
						}
					}
				})
				this.current.cis.strany.filter(x => x.VSTRANA === party.VSTRANA).forEach(res => {
					...
					if (runs) {
						...
						party.current = {
							party: res,
							result: this.current.list[0].$vysledky.find(x => x.ESTRANA === runs.ESTRANA),
							list: this.current.list[0].$zvoleni.filter(x => x.ESTRANA === runs.ESTRANA),
							runs
						}
					}
				})
		...
		candidates_renewed: function () {
			var list = [];
			var parties = this.current.list[0].$strany.filter(x => x.POCMANDCR > 0);

			this.current.list[0].$zvoleni.forEach(elected => {
				var o = {
					display: elected.JMENO + ' ' + elected.PRIJMENI,
					currentParty: parties.find(x => x.ESTRANA === elected.ESTRANA),
				...
				this.previous.list[0].$zvoleni.filter(x => x.JMENO === elected.JMENO && x.PRIJMENI === elected.PRIJMENI && x.VEK < elected.VEK).forEach(prev => {
					o.previousParty = this.previous.list[0].$strany.find(x => x.ESTRANA === prev.ESTRANA);
		...
		candidates_lost: function () {
			var list = [];
			var parties = this.previous.list[0].$strany.filter(x => x.POCMANDCR > 0);

			this.previous.list[0].$zvoleni.forEach(elected => {
				var o = {
					display: elected.JMENO + ' ' + elected.PRIJMENI,
					previousParty: parties.find(x => x.ESTRANA === elected.ESTRANA),
				...
				this.current.list[0].$zvoleni.filter(x => x.JMENO === elected.JMENO && x.PRIJMENI === elected.PRIJMENI && x.VEK > elected.VEK).forEach(prev => {
					o.currentParty = this.previous.list[0].$strany.find(x => x.ESTRANA === prev.ESTRANA);
